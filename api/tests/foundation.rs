...
async fn pool_connects_and_migrates() {
    dotenvy::dotenv().ok();
    let config = onecortex_vector_api::config::AppConfig::from_env().unwrap();
    ...
    .await
    .unwrap();
    assert!(row.0, "_onecortex_vector schema should exist after migrations");
}
...
async fn lifecycle_create_and_drop() {
    ...
    let schema_name = onecortex_vector_api::db::lifecycle::schema_name_for(index_id);
    ...
    let (status,): (String,) = sqlx::query_as(
        "SELECT status FROM _onecortex_vector.indexes WHERE id = $1"
    )
    .bind(index_id)
    .fetch_one(&pool)
    .await
    ...
    // Drop the schema
    onecortex_vector_api::db::lifecycle::drop_index_schema(&pool, index_id, &schema_name)
        .await.unwrap();
    ...
    let row: Option<(Uuid,)> = sqlx::query_as(
        "SELECT id FROM _onecortex_vector.indexes WHERE id = $1"
    )
    .bind(index_id)
    .fetch_optional(&pool)
    .await
