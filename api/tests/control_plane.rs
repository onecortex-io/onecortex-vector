...
async fn create_index_success() {
    ...
    let name = format!("cp-create-{}", &uuid::Uuid::new_v4().simple().to_string()[..8]);
...
async fn describe_index_not_found() {
    ...
    let resp = client.get(format!("{}/indexes/nonexistent-index", server.base_url))
        .header("Api-Key", &server.api_key)
        .send().await.unwrap();
...
async fn delete_index() {
    ...
    let resp = client.delete(format!("{}/indexes/{name}", server.base_url))
        .header("Api-Key", &server.api_key)
        .send().await.unwrap();
...
async fn deletion_protection_blocks_delete() {
    ...
    client.patch(format!("{}/indexes/{name}", server.base_url))
        .header("Api-Key", &server.api_key)
        .json(&json!({ "deletion_protection": "enabled" }))
        .send().await.unwrap();
...
async fn describe_index_stats() {
    ...
    let resp = client.post(format!("{}/indexes/{name}/describe_index_stats", server.base_url))
        .header("Api-Key", &server.api_key)
        .json(&json!({}))
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
