...
async fn upsert_and_fetch() {
    ...
    // Upsert
    let resp = client.post(format!("{}/indexes/{name}/vectors/upsert", server.base_url))
    ...
    // Fetch
    let resp = client.post(format!("{}/indexes/{name}/vectors/fetch", server.base_url))
        .header("Api-Key", &server.api_key)
        .json(&json!({ "ids": ["v1", "v2"] }))
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
...
async fn upsert_batch_too_large() {
    ...
    let vectors: Vec<serde_json::Value> = (0..1001).map(|i| {
        json!({"id": format!("v{i}"), "values": [1.0, 0.0, 0.0]})
    }).collect();
...
async fn fetch_by_metadata_eq() {
    ...
    let resp = client.post(format!("{}/indexes/{name}/vectors/fetch_by_metadata", server.base_url))
        .header("Api-Key", &server.api_key)
...
async fn delete_by_ids() {
    ...
    // Delete v1
    client.post(format!("{}/indexes/{name}/vectors/delete", server.base_url))
        .header("Api-Key", &server.api_key)
        .json(&json!({ "ids": ["v1"] }))
        .send().await.unwrap();
...
async fn update_metadata() {
    ...
    let resp = client.post(format!("{}/indexes/{name}/vectors/update", server.base_url))
        .header("Api-Key", &server.api_key)
...
async fn list_vectors_with_prefix() {
    ...
    let resp = client.get(format!("{}/indexes/{name}/vectors/list?prefix=doc-", server.base_url))
        .header("Api-Key", &server.api_key)
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
...
async fn list_vectors_pagination() {
    ...
    // First page: limit=2
    let resp = client.get(format!("{}/indexes/{name}/vectors/list?limit=2", server.base_url))
        .header("Api-Key", &server.api_key)
        .send().await.unwrap();
    let body: serde_json::Value = resp.json().await.unwrap();
    let vectors = body["vectors"].as_array().unwrap();
    ...
    // Second page
    let resp = client.get(format!("{}/indexes/{name}/vectors/list?limit=2&paginationToken={next}", server.base_url))
        .header("Api-Key", &server.api_key)
        .send().await.unwrap();
    let body: serde_json::Value = resp.json().await.unwrap();
    let vectors = body["vectors"].as_array().unwrap();
