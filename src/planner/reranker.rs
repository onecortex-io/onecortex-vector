...
impl Reranker for CohereReranker {
    async fn rerank(
        ...
        let mut results = map_indexed_results(
            resp.results.iter().map(|r| (r.index, r.relevance_score)),
            &candidates,
        );
        results.sort_by(|a, b| {
            b.rerank_score
...
impl Reranker for VoyageReranker {
    async fn rerank(
        ...
        let mut results = map_indexed_results(
            resp.data.iter().map(|r| (r.index, r.relevance_score)),
            &candidates,
        );
        results.sort_by(|a, b| {
            b.rerank_score
...
impl Reranker for PineconeReranker {
    async fn rerank(
        ...
        let mut results =
            map_indexed_results(resp.data.iter().map(|r| (r.index, r.score)), &candidates);
        results.sort_by(|a, b| {
            b.rerank_score
                .partial_cmp(&a.rerank_score)
                .unwrap_or(std::cmp::Ordering::Equal)
...
impl Reranker for CrossEncoderReranker {
    async fn rerank(
        ...
        let mut results =
            map_indexed_results(tei_results.iter().map(|r| (r.index, r.score)), &candidates);
        results.sort_by(|a, b| {
            b.rerank_score
                .partial_cmp(&a.rerank_score)
                .unwrap_or(std::cmp::Ordering::Equal)
...
pub fn build_reranker(config: &AppConfig) -> Arc<dyn Reranker> {
    ...
    match config.rerank_backend.as_str() {
        "cohere" => {
            let key = config.rerank_cohere_api_key.clone().expect(
                "ONECORTEX_VECTOR_RERANK_COHERE_API_KEY must be set when rerank_backend=cohere",
            );
            Arc::new(CohereReranker::new(
                key,
                config.rerank_cohere_model.clone(),
                timeout,
                retries,
            ))
        }
        "voyage" => {
            let key = config.rerank_voyage_api_key.clone().expect(
                "ONECORTEX_VECTOR_RERANK_VOYAGE_API_KEY must be set when rerank_backend=voyage",
            );
            Arc::new(VoyageReranker::new(
                key,
                config.rerank_voyage_model.clone(),
                timeout,
                retries,
            ))
        }
        "jina" => {
            let key = config.rerank_jina_api_key.clone().expect(
                "ONECORTEX_VECTOR_RERANK_JINA_API_KEY must be set when rerank_backend=jina",
            );
            Arc::new(JinaReranker::new(
                key,
                config.rerank_jina_model.clone(),
                timeout,
                retries,
            ))
        }
        "pinecone" => {
            let key = config.rerank_pinecone_api_key.clone().expect(
                "ONECORTEX_VECTOR_RERANK_PINECONE_API_KEY must be set when rerank_backend=pinecone",
            );
            Arc::new(PineconeReranker::new(
                key,
                config.rerank_pinecone_model.clone(),
                timeout,
                retries,
            ))
        }
